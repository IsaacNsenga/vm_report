...
class RAPPORT_GSMAdmin(admin.ModelAdmin):
    def get_queryset(self, request):
        ...
        if request.user.has_perm('app_name.can_view_all'):  
            return qs  # Directeur Général voit tout
        elif request.user.has_perm('app_name.can_view_all_gsm'):
            return qs  # Responsable GSM voit tout
        elif request.user.has_perm('app_name.can_view_own_gsm'):
...
class RAPPORT_MOMOPAYAdmin(admin.ModelAdmin):
    def get_queryset(self, request):
        ...
        if request.user.has_perm('app_name.can_view_all'):  
            return qs  # Directeur Général voit tout
        elif request.user.has_perm('app_name.can_view_all_momopay'):
            return qs  # Responsable MoMoPay voit tout
        elif request.user.has_perm('app_name.can_view_own_momopay'):
